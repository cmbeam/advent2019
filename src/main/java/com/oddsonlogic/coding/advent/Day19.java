    ...
    public static void main(String[] args) {
        ...
        if(SHOW_GRAPH) {
            ...
            for (int i = 0; i < GRID_SIZE; i++) {
                ...
                for (int j = 0; j < GRID_SIZE; j++) {
                    int inBeamCount = 0;
                    computer.reset();
                    computer.inputs.add(j);
                    computer.inputs.add(i);
                    int map = computer.compute().intValue();
                    if (map == 1) {
                        System.out.print("#");
        ...
        for (int i = 0; i < GRID_SIZE; i++) {
            ...
            for (int j = 0; j < GRID_SIZE; j++) {
                ...
                System.out.println("coordinate check: "+j+","+i);
                computer.reset();
                computer.inputs.add(j);
                computer.inputs.add(i);
                int map1 = computer.compute().intValue();
                ...
                if (map1 == 1) {
                    ...
                    previousOn = true;
                    computer.reset();
                    computer.inputs.add(j+SHIP_SIZE-1);
                    computer.inputs.add(i);
                    int map2 = computer.compute().intValue();

                    if (map2 == 1) {
                        computer.reset();
                        computer.inputs.add(j);
                        computer.inputs.add(i+SHIP_SIZE-1);
                        int map3 = computer.compute().intValue();

                        if (map3 == 1) {
                            computer.reset();
                            computer.inputs.add(j+SHIP_SIZE-1);
                            computer.inputs.add(i+SHIP_SIZE-1);
                            int map4 = computer.compute().intValue();
